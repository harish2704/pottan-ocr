...
def KerasCrnn(imgH=32, nc=1, nclass=134, nh=64 ):
    ...
    def convRelu(i, batchNormalization=False):
        ...
        if( i == 0):
            cnn.add( ZeroPadding2D(padding=1, input_shape=( 1, 32, 32 ) ) )
            cnn.add( Conv2D( nOut, ks[i], strides=ss[i], padding='valid', name='conv{0}'.format(i) ) )
        else:
            if( ps[i] != 0 ):
                cnn.add( ZeroPadding2D(padding=ps[i] ) )
            cnn.add( Conv2D( nOut, ks[i], strides=ss[i], padding='valid', name='conv{0}'.format(i) ) )
        if batchNormalization:
            cnn.add( BatchNormalization(axis=1, momentum=0.1, epsilon=1e-5, name='batchnorm{0}'.format(i) ))
